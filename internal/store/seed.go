...
var defaultAnalyzerPrompt = `# 角色定义
...
var defaultArchitectPrompt = `# 角色定义
...
本系统的最终产出物是**一组可直接使用的提示词**。用户拿到这些提示词后，会按顺序配合AI工具使用：
- 提示词A的输出 → 作为提示词B的输入
...
var defaultReviewerPrompt = `# 角色定义
...
func SeedDefaults(db *gorm.DB) error {
	seeds := []model.Template{
		{
			Name:        "默认分析师",
			Description: "需求深度分析与策略规划",
			Stage:       "analyzer",
		...
		{
			Name:        "默认撰写师",
			Description: "领域专家级提示词撰写",
			Stage:       "writer",
			Prompt:      defaultWriterPrompt,
			Version:     1,
			IsDefault:   true,
		},
		{
			Name:        "默认审稿人",
			Description: "提示词质量审核与修订",
			Stage:       "reviewer",
	...
	for _, s := range seeds {
		...
		err := db.Where("stage = ? AND is_default = ?", s.Stage, true).First(&existing).Error
		if err != nil {
			if err := db.Create(&s).Error; err != nil {
				return err
			}
		} else if existing.Prompt != s.Prompt {
	...
	// 修复旧数据：有结果但 status 仍为 running 的历史记录标记为 done
	db.Model(&model.History{}).
		Where("status = 'running' AND duration_ms > 0").
